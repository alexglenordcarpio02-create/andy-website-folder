...
const statsObserver = new IntersectionObserver((entries) => {
  entries.forEach(e => {
    if (e.isIntersecting) { animateCounter(e.target, +e.target.dataset.target); statsObserver.unobserve(e.target); }
  });
}, { threshold: 0.5 });
...
function createParticle() {
  return { x: Math.random() * canvas.width, y: Math.random() * canvas.height, r: Math.random() * 2 + .5, vx: (Math.random() - .5) * .4, vy: (Math.random() - .5) * .4, alpha: Math.random() * .5 + .1, color: Math.random() > .6 ? '#3dbcb0' : '#f6c94e' };
}
...
function drawParticles() {
  ...
  particles.forEach((p, i) => {
    ...
    if (dist < 100) { const force = (100-dist)/100; p.x += dx*force*.02; p.y += dy*force*.02; }
    ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI*2);
    ctx.fillStyle = p.color; ctx.globalAlpha = p.alpha; ctx.fill();
    for (let j = i+1; j < particles.length; j++) {
      const q = particles[j], ddx = p.x-q.x, ddy = p.y-q.y, d = Math.sqrt(ddx*ddx+ddy*ddy);
      if (d < 120) { ctx.beginPath(); ctx.moveTo(p.x,p.y); ctx.lineTo(q.x,q.y); ctx.strokeStyle='#3dbcb0'; ctx.globalAlpha=(1-d/120)*.12; ctx.lineWidth=.8; ctx.stroke(); }
    }
    ctx.globalAlpha = 1;
  });
...
if (book3dWrap) {
  ...
  book3dWrap.addEventListener('mousemove', e => {
    ...
    const rotY = ((e.clientX - rect.left - rect.width/2) / rect.width) * 30;
    const rotX = -((e.clientY - rect.top - rect.height/2) / rect.height) * 15;
    book3d.style.transform = `rotateY(${-15+rotY}deg) rotateX(${5+rotX}deg)`;
  });
  book3dWrap.addEventListener('mouseleave', () => { book3d.style.transform = 'rotateY(-15deg) rotateX(5deg)'; });
}
